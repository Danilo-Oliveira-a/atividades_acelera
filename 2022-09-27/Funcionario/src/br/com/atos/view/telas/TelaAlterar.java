...
public class TelaAlterar {
	...
	public void alterar(JFrame frameMenuInicial, JFrame frameAnterior,
		...
		if(tipoFuncionario.equals(TipoFuncionario.GERENTE.getDescricao())) {
			existeFuncionario = repositorioGerenteImpl.listarGerentes().stream()
					.filter(Objects::nonNull)
					.filter(f->f.getCpf().equals(valorCpf)).findFirst().isPresent();
			if(existeFuncionario) {
				Gerente func = repositorioGerenteImpl.listarGerentes().stream()
						.filter(Objects::nonNull)
			...
			existeFuncionario = repositorioCoordenadorImpl.listarCoordenadores().stream()
					.filter(Objects::nonNull)
					.filter(f->f.getCpf().equals(valorCpf)).findFirst().isPresent();
			if(existeFuncionario) {
				Coordenador func = repositorioCoordenadorImpl.listarCoordenadores().stream()
						.filter(Objects::nonNull)
						.filter(f->f.getCpf().equals(valorCpf)).findFirst().get();
				nome = func.getNome();
				cpf=func.getCpf();
